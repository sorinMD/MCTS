        ...
        implements VectorConstants, HexTypeConstants, GameStateConstants 
{
    ...
    public void setBoardLayoutSize(){
...
//    public final static int PORT_STONE      = 6;    
    ...
    void DrawVertex(Graphics2D g2, int i)
    {
        ...
        vstate = boardlayout.getState()[OFS_VERTICES+i];
...
//        g2.drawString(""+i, v.screenCoord.x-W, v.screenCoord.y+W);
    ...
    void DrawEdge(Graphics2D g2, int i)
    {
        ...
        estate = boardlayout.getState()[OFS_EDGES+i];
...
//    }
    ...
    void DrawBoard(Graphics g)
    {
       ...
       int i;
        ...
        g2.setFont(font2);
        val = boardlayout.getState()[OFS_DICE];
        g2.drawRect(5, 5, 40, 25);
        if (val==0)
            s = "?";
        else
            s = "" + val;
        ...
        //current player in the main game, not in the fsm
        int fsmlevel = boardlayout.getState()[OFS_FSMLEVEL];
        val = boardlayout.getState()[OFS_FSMPLAYER+fsmlevel];
        g2.drawRect(50+5, 5, 25, 25);
        s = "" + val;
        g2.drawString(s, 50+15, 25);
    ...
    void DrawPlayerInfo(Graphics2D g2, int pl)
    {
        ...
        g2.setColor(Color.BLACK);
        g2.setFont(font2);
        val = boardlayout.getState()[OFS_PLAYERDATA[pl] + OFS_SCORE];
        g2.drawString(""+val, x0+col1ofs+10, y0 + 5*rowheight+10);
        
        g2.setColor(Color.BLACK);
        g2.setFont(font3);
        val = boardlayout.getState()[OFS_LONGESTROAD_AT];
        if (val==pl)
            g2.setColor(Color.BLACK);
        else
            g2.setColor(Color.LIGHT_GRAY);
        g2.drawString("Longest road: " + boardlayout.getState()[OFS_PLAYERDATA[pl]+OFS_PLAYERSLONGESTROAD], x0+col1ofs, y0 + 7*rowheight);
        val = boardlayout.getState()[OFS_LARGESTARMY_AT];
        if (val==pl)
        {
            g2.setColor(Color.BLACK);
            g2.drawString("Largest army: " + boardlayout.getState()[OFS_PLAYERDATA[pl]+OFS_USEDCARDS+CARD_KNIGHT],
                    x0+col1ofs, y0 + 8*rowheight);
        }
        else
        {
            g2.setColor(Color.LIGHT_GRAY);
            g2.drawString("Army size: " + boardlayout.getState()[OFS_PLAYERDATA[pl]+OFS_USEDCARDS+CARD_KNIGHT],
                    x0+col1ofs, y0 + 8*rowheight);
        }
        ...
        {
            val = boardlayout.getState()[OFS_PLAYERDATA[pl]+OFS_RESOURCES+i];
        ...
        {
            val = boardlayout.getState()[OFS_PLAYERDATA[pl]+OFS_ACCESSTOPORT+i];
        ...
        {
            str = "";
            val = boardlayout.getState()[OFS_PLAYERDATA[pl]+OFS_OLDCARDS+i];
            str = str + val;
            val = boardlayout.getState()[OFS_PLAYERDATA[pl]+OFS_NEWCARDS+i];
            str = str + " (+" +val+ ")";
            val = boardlayout.getState()[OFS_PLAYERDATA[pl]+OFS_USEDCARDS+i];
            str = str + " u:"+val;
